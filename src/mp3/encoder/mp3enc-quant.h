#pragma once
// mp3enc-quant.h
// ISO 11172-3 Layer III quantization and nested iteration loops.
...
// Dequant matches standard: xr = |ix|^(4/3) * 2^((gg-210)/4) * 2^(-ss*sf)
...
static int mp3enc_inner_loop(const float *         xr,
    ...
    if (best_bits > available_bits) {
        // No gain fit the budget! Use maximum gain to minimize bits.
        best_gain = 255;
    }
    ...
    // Emergency safeguard: if it still overflows the physical budget, zero out lines from top to bottom
    // to prevent the bitstream from being truncated and corrupted.
    int orig_best_bits = best_bits;
    while (best_bits > available_bits) {
        int last_nz = 575;
        while (last_nz >= 0 && ix[last_nz] == 0) {
            last_nz--;
        }
        if (last_nz < 0) break;
        
        int start = (last_nz > 16) ? last_nz - 16 : 0;
    ...
    static int p_cnt = 0;
    if (p_cnt < 20) {
        FILE* f = fopen("debug_bits.txt", "a");
        if (f) {
            fprintf(f, "BUDGET: avail=%d, orig_best=%d, final_best=%d, gain=%d\n", available_bits, orig_best_bits, best_bits, best_gain);
            fclose(f);
        }
        p_cnt++;
    }
...
static int mp3enc_outer_loop(const float *         xr,
    ...
    int orig_best_gain = gi.global_gain;
    ...
    for (int iter = 0; iter < 25; iter++) {
        ...
        // 3-axis quant_compare (inspired by advanced outer_loop models):
        ...
        if (dump_cnt < 2 && gr == 0 && iter == 0) {
            FILE* f = fopen("debug_thrash.txt", "a");
            if (f) {
                fprintf(f, "FRAME %d iter %d gain %d orig %d\n", dump_cnt, iter, gi.global_gain, orig_best_gain);
                for(int sfb=0; sfb<21; sfb++) {
                    if (sfb_table[sfb]==0) break;
                    fprintf(f, " sfb %d: noise %f xmin %f sf %d\n", sfb, noise[sfb], xmin[sfb], gi.scalefac_l[sfb]);
                }
                fclose(f);
            }
            dump_cnt++;
        ...
        for (int sfb = 0; sfb < 21; sfb++) {
            ...
            for (int j = 0; j < width; j++) {
                if (ix[pos + j] >= 8191 || ix[pos + j] <= -8191) {
                    clipped = true;
                }
            }
            
            if (xmin[sfb] > 0.0f && noise[sfb] > xmin[sfb]) {
                if (gi.scalefac_l[sfb] < 15 && !clipped) {
                    gi.scalefac_l[sfb]++;
                    any_changed = true;
                    all_ok = false;
                }
        ...
        // Anti-thrashing safeguard (fixes clipping at scalefac=15)
        // If we increased scalefactors, we must check if the inner loop will
        // just increase global_gain to compensate. If it does, we are at the
        // physical bit ceiling. We should stop here.
        if (gi.global_gain > orig_best_gain + 2) { 
            break;
        }
    ...
    static int f_cnt = 0;
    if (f_cnt == 100 && gr == 0) {
        FILE* f = fopen("debug_final.txt", "w");
        if (f) {
            fprintf(f, "FRAME 100\nGAIN %d\nSCALEFACS:\n", gi.global_gain);
            for(int i=0; i<21; i++) fprintf(f, "%d ", gi.scalefac_l[i]);
            fprintf(f, "\nIX MAX: ");
            int m = 0;
            for(int i=0; i<576; i++) { if (abs(ix[i])>m) m = abs(ix[i]); }
            fprintf(f, "%d\n", m);
            fclose(f);
        }
    }

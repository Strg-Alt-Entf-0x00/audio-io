#pragma once
// mp3enc-psy.h
// Psychoacoustic Model 2 for MPEG-1 Audio Layer III.
//
// Computes masking thresholds (xmin) and perceptual entropy (PE) for bit
// allocation. It relies on the MDCT coefficients directly, bypassing the 1024-point
// FFT required by the exact standard. This sacrifices some precision in tone
// masking but significantly reduces complexity and latency, while still providing
// good bit allocation hints.
//
...
// Configurable magic numbers for the Psychoacoustic Model
struct mp3enc_psy_config {
    float tmn;               // Tonal Masking Noise (default 13.0)
    float nmt;               // Noise Masking Tone (default 4.5)
    float ath_shift;         // ATH floor shift (default 0.0 for 16-bit PCM scale)
    float spread_up_slope;   // Upward masking slope multiplier (default 1.35)
    float spread_down_slope; // Downward masking slope multiplier (default 2.7)
    float fwd_decay;         // Forward temporal masking decay (default 0.063)
    float pre_echo_mult1;    // Pre-echo 1-frame history multiplier (default 2.0)
    
    mp3enc_psy_config() {
        // Original safe defaults (ISO/Shine base)
    tmn = 13.0f;
    nmt = 4.5f;
    ath_shift = 0.0f; // Scaled to match MDCT energy (max 10^9) -> ATH at 1.0 is -90dB
    spread_up_slope = 1.35f;
    spread_down_slope = 2.7f;
    fwd_decay = 0.063f;
    pre_echo_mult1 = 2.0f;

        // Override via environment variables
        if (const char* env_p = std::getenv("AUDIO_IO_PSY_TMN")) tmn = std::stof(env_p);
        if (const char* env_p = std::getenv("AUDIO_IO_PSY_NMT")) nmt = std::stof(env_p);
        if (const char* env_p = std::getenv("AUDIO_IO_PSY_ATH_SHIFT")) ath_shift = std::stof(env_p);
        if (const char* env_p = std::getenv("AUDIO_IO_PSY_SPREAD_UP")) spread_up_slope = std::stof(env_p);
        if (const char* env_p = std::getenv("AUDIO_IO_PSY_SPREAD_DOWN")) spread_down_slope = std::stof(env_p);
        if (const char* env_p = std::getenv("AUDIO_IO_PSY_FWD_DECAY")) fwd_decay = std::stof(env_p);
        if (const char* env_p = std::getenv("AUDIO_IO_PSY_PRE_ECHO")) pre_echo_mult1 = std::stof(env_p);
    }
};

// Psychoacoustic model state.
struct mp3enc_psy {
    mp3enc_psy_config config;
    ...
    float prev_energy[2]; // total MDCT energy from previous frame per channel
    ...
    void init_ath(int sr_index, const uint8_t * sfb_table, int sample_rate) {
        ...
        for (int sfb = 0; sfb < MP3ENC_PSY_SFB_MAX; sfb++) {
            ...
            float ath_linear          = powf(10.0f, (ath_min_db - config.ath_shift) * 0.1f) * (float) width;
    ...
    void compute(const float * mdct, const uint8_t * sfb_table, int sr_index, int ch = 0) {
        ...
        // PE boost for short blocks is handled later by giving more bits.
        float offset_linear[MP3ENC_PSY_SFB_MAX];
        for (int sfb = 0; sfb < MP3ENC_PSY_SFB_MAX; sfb++) {
            float alpha        = tonality[sfb];
            float offset_db    = alpha * config.tmn + (1.0f - alpha) * config.nmt;
        ...
        for (int j = 0; j < MP3ENC_PSY_SFB_MAX; j++) {
            ...
            for (int i = 0; i < MP3ENC_PSY_SFB_MAX; i++) {
                ...
                float dz = (dz_raw >= 0.0f) ? dz_raw * config.spread_up_slope : dz_raw * config.spread_down_slope;
        ...
        for (int sfb = 0; sfb < MP3ENC_PSY_SFB_MAX; sfb++) {
            ...
            float clamped = nb;
            float lim1    = config.pre_echo_mult1 * nb_1[ch][sfb];
        ...
        for (int sfb = 0; sfb < MP3ENC_PSY_SFB_MAX; sfb++) {
            float fwd = prev_mask[ch][sfb] * config.fwd_decay;
            if (fwd > xmin[sfb]) {
                xmin[sfb] = fwd;
            }
        ...
        // Re-compute energy ratio for PE boost
        float total_energy = 0.0f;
        for (int i = 0; i < 576; i++) {
            total_energy += mdct[i] * mdct[i];
        }
        float energy_ratio = 1.0f;
        if (total_energy > 1e-6f && prev_energy[ch] > 1e-6f) {
            energy_ratio = total_energy / prev_energy[ch];
        }

        // Apply pre-echo control to PE:
        // If energy spiked (transient attack), boost the PE proportional to the spike.
        // This forces the bit allocator to drain the bit reservoir for this frame,
        // preventing quantization noise from leaking before the attack.
        if (energy_ratio > 4.0f) {
            float boost = sqrtf(energy_ratio) * 0.5f;
            if (boost > 3.0f) boost = 3.0f; // max 3x PE boost
            pe *= boost;
        }
    }
};

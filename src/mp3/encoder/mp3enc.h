...
static mp3enc_t *mp3enc_init(int sample_rate, int channels, int bitrate_kbps,
  ...
  enc->is_vbr = (vbr_max > 0);
  enc->vbr_min_kbps = bitrate_kbps;
  enc->vbr_max_kbps = vbr_max;
  enc->vbr_quality = vbr_qual;
  ...
  // Cutoff per total bitrate in Hz. Higher bitrates preserve more bandwidth.
  {
    static const struct {
      int kbps;
      int hz;
    } lp_table[] = {{8, 2000},    {16, 3700},   {24, 3900},   {32, 5500},
                    {40, 7000},   {48, 7500},   {56, 10000},  {64, 11000},
                    {80, 13500},  {96, 15100},  {112, 15600}, {128, 17000},
                    {160, 17500}, {192, 18600}, {224, 19400}, {256, 19700},
                    {320, 20500}};
...
static int mp3enc_encode_frame(mp3enc_t *enc, const float *pcm) {
  ...
  // Since MS is currently broken, we force mode=0 for safe L/R.
  int mode = (nch == 1) ? 3 : 0;
  int mode_ext = 0; // set after MDCT energy analysis

  if (enc->is_vbr) {
    // ABSVBR: Adaptive Bit Allocation based on perceptual entropy
    float pe_total = enc->psy.pe;
    int target = enc->vbr_min_kbps;
    if (pe_total > 1000.0f)
      target += 32;
    if (pe_total > 2000.0f)
      target += 64;
    if (pe_total > 3000.0f)
      target += 128;
    target = static_cast<int>(target * enc->vbr_quality * 2.0f);
    if (target > enc->vbr_max_kbps)
      target = enc->vbr_max_kbps;
    if (target < enc->vbr_min_kbps)
      target = enc->vbr_min_kbps;

    static const int brTable[] = {32,  40,  48,  56,  64,  80,  96,
                                  112, 128, 160, 192, 224, 256, 320};
    int best_br = brTable[0];
    for (int i = 0; i < 14; i++) {
      if (brTable[i] <= target)
        best_br = brTable[i];
    }
    enc->bitrate_kbps = best_br;
  }
  ...
  // Unused bytes at the end of the previous frame can be borrowed by this
  // frame. The pending_frame holds the previous frame; its unused tail is the
  // reservoir.
  int resv_bytes = 0;
  ...
  for (int gr = 0; gr < 2; gr++) {
    ...
    for (int ch = 0; ch < nch; ch++) {
      ...
      // Transient detection (Short blocks)
      float total_energy = 0.0f;
      for (int slot = 0; slot < 18; slot++) {
        for (int sb = 0; sb < 32; sb++) {
          float v = enc->sb_cur[ch][sb][slot];
          total_energy += v * v;
        }
      }

      int block_type = 0; // Forced to LONG_BLOCK for now
      // Short block quantization is missing in mp3enc-quant.h.
      // If we enable short blocks here, the MDCT is short, but the bitstream
      // formats it as long, causing complete audio corruption.
      enc->psy.prev_energy[ch] = total_energy;

      si.gr[gr][ch].block_type = block_type;
      si.gr[gr][ch].mixed_block_flag = 0;
    ...
    // DEBUG: print mdct max value for channel 0
    if (gr == 0) {
        static int debug_frame = 0;
        if (debug_frame < 5) {
            float mmax = 0.0f;
            for(int i=0; i<576; i++) {
                if(abs(mdct_all[gr][0][i]) > mmax) mmax = abs(mdct_all[gr][0][i]);
            }
            FILE* f = fopen("debug_mdct.txt", "a");
            if (f) {
                fprintf(f, "DEBUG FRAME %d: MDCT MAX = %f\n", debug_frame, mmax);
                fclose(f);
            }
            debug_frame++;
        }
  ...
  // that the side channel is cheap to encode. ISO/IEC 11172-3 suggests it is
  // beneficial for highly correlated stereo signals. We use it unless side energy dominates,
  // which means the channels are very different (rare for music).
  bool use_ms = false;
  if (nch == 2) {
    // Force L/R stereo because M/S is buggy in the quantizer loop right now.
    // Wait for the author to fix the MS quantizer before enabling.
    use_ms = false;
    mode_ext = use_ms ? 2 : 0;
    hdr.mode_ext = mode_ext;
  }
  ...
  // signal). We use the Perceptual Entropy to drive the bit reservoir. We weight the per-granule
  ...
  for (int gr = 0; gr < 2; gr++) {
    ...
    int gr_bits_used = 0;
    for (int ch = 0; ch < nch; ch++) {
      int bits = mp3enc_outer_loop(mdct_all[gr][ch], ix[gr][ch], si.gr[gr][ch],
                                   saved_xmin[gr][ch], bits_per_ch, sfb_long,
    ...
    for (int ch = 0; ch < nch; ch++) {
      ...
      for (int r = 0; r < 3; r++) {
        ...
        for (int p = 0; p < pairs; p++) {
          int i = prev_end + p * 2;
          mp3enc_write_pair(md_bs, gi.table_select[r], ix[gr][ch][i],
                            ix[gr][ch][i + 1]);
        }
        prev_end = reg_end;
      }
      ...
      mp3enc_write_count1(md_bs, ix[gr][ch], c1_start, c1_count,
                          gi.count1table_select);
    }
  }
  int md_bytes = (md_bs.total_bits() + 7) / 8;

  // Assemble output: write overflow to pending frame, output it, save current
  // as pending.
  //
  ...
  if (enc->pending_bytes > 0) {
    ...
    if (overflow > 0) {
      memcpy(enc->pending_frame + enc->pending_md_end, enc->md_scratch,
             overflow);
    }
    ...
    memcpy(enc->out_buf + enc->out_written, enc->pending_frame,
           enc->pending_bytes);
    enc->out_written += enc->pending_bytes;
    output_bytes = enc->pending_bytes;
  }

  // Build current frame: header+sideinfo already in frame_buf, add main_data +
  // padding
  ...
  if (md_in_frame > 0) {
    memcpy(enc->frame_buf + hdr_si_bytes, enc->md_scratch + resv_bytes,
           md_in_frame);
  }
  // Zero-pad the unused tail (this becomes reservoir for the next frame)
  if (md_in_frame < md_area) {
    memset(enc->frame_buf + hdr_si_bytes + md_in_frame, 0,
           md_area - md_in_frame);
  }
...
static const uint8_t *mp3enc_encode(mp3enc_t *enc, const float *audio,
                                    int n_samples, int *out_size) {
  *out_size = 0;
  int nch = enc->channels;
  int consumed = 0;
  ...
  while (consumed < n_samples) {
    ...
    for (int ch = 0; ch < nch; ch++) {
      memcpy(enc->pcm_buf + ch * 1152 + enc->pcm_fill,
             audio + ch * n_samples + consumed, copy * sizeof(float));
    }
    enc->pcm_fill += copy;
    consumed += copy;
...
static const uint8_t *mp3enc_flush(mp3enc_t *enc, int *out_size) {
  *out_size = 0;
  ...
  if (enc->pcm_fill > 0) {
    ...
    int nch = enc->channels;
    for (int ch = 0; ch < nch; ch++) {
      memset(enc->pcm_buf + ch * 1152 + enc->pcm_fill, 0,
             (1152 - enc->pcm_fill) * sizeof(float));
    }
    enc->pcm_fill = 1152;
    mp3enc_encode_frame(enc, enc->pcm_buf);
    enc->pcm_fill = 0;
